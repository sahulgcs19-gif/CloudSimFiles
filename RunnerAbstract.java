...
import org.cloudbus.cloudsim.power.PowerVmAllocationPolicyMigrationMOHD;
...
import org.cloudbus.cloudsim.power.PowerVmSelectionPolicyMinimumPredictedMemory;
...
public abstract class RunnerAbstract {
	...
	protected VmAllocationPolicy getVmAllocationPolicy(
		...
		if (vmAllocationPolicyName.equals("iqr")) {
			...
			vmAllocationPolicy = new PowerVmAllocationPolicyMigrationMedianAbsoluteDeviation(
					...
					parameter,
					fallbackVmSelectionPolicy);
		}else if (vmAllocationPolicyName.equals("mohd")) {
			final double epsilon          = 0.20; // tolerance level (20%)
		    final int    baseWindow       = 5;    // starting WMA window
		    final double theta            = 0.05; // deviation threshold to grow/shrink window
		    final double gammaCpu         = 0.90; // CPU overutilization threshold (90%)
		    final double memThresholdFrac = 0.95; // memory overutilization threshold (95% of host RAM)
			PowerVmAllocationPolicyMigrationAbstract fallbackVmSelectionPolicy = new PowerVmAllocationPolicyMigrationStaticThreshold(
					hostList,
					vmSelectionPolicy,
					0.7);
			vmAllocationPolicy = new PowerVmAllocationPolicyMigrationMOHD(
					hostList,
					vmSelectionPolicy,
					epsilon,baseWindow,theta, gammaCpu, memThresholdFrac,
					fallbackVmSelectionPolicy);
	...
	protected PowerVmSelectionPolicy getVmSelectionPolicy(String vmSelectionPolicyName) {
		...
		} else if (vmSelectionPolicyName.equals("mmtrr")) {
			vmSelectionPolicy = new PowerVmSelectionPolicyMinimumPredictedMemory();
		} else {
