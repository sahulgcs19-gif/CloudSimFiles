...
public class Helper {
	...
	public static List<Vm> createVmList(int brokerId, int vmsNumber) {
	    ...
	    // >>> Added line (use your predefined per-type counts)
	    final int[] COUNTS = {119,119,119,119,119};

	    // (Optional) sanity: ensure requested VM count matches the counts array sum
	    int total = 0; for (int c : COUNTS) total += c;
	    if (vmsNumber != total) {
	        throw new IllegalArgumentException("vmsNumber (" + vmsNumber + ") != sum(VM_COUNTS_100K_CORES) (" + total + ")");
	    }

	    int id = 0;
	    for (int vmType = 0; vmType < Constants.VM_TYPES; vmType++) {
	        for (int k = 0; k < COUNTS[vmType]; k++) {
	            vms.add(new PowerVm(
	            	vmType,
	                id++,
	                brokerId,
	...
	public static List<PowerHost> createHostList(int hostsNumber) {
	    // Required counts: 18-core => 1848, 16-core => 2087, 64-core => 521
	    final int need18 = 195, need16 = 219, need64 = 119;
	    final int total = need18 + need16 + need64; // 4456

	    if (hostsNumber != total) {
	        throw new IllegalArgumentException(
	            "hostsNumber (" + hostsNumber + ") must be " + total +
	            " for the requested mix: 18-core=" + need18 +
	            ", 16-core=" + need16 + ", 64-core=" + need64
	        );
	    }

	    int[] countsByType = new int[Constants.HOST_TYPES];
	    countsByType[0] = need18;
	    countsByType[1] = need64;
	    countsByType[2] = need16;

	    List<PowerHost> hostList = new ArrayList<PowerHost>();
	    int id = 0;

	    for (int hostType = 0; hostType < Constants.HOST_TYPES; hostType++) {
	        int howMany = countsByType[hostType];
	        for (int k = 0; k < howMany; k++) {

	            List<Pe> peList = new ArrayList<Pe>();
	            for (int j = 0; j < Constants.HOST_PES[hostType]; j++) {
	                peList.add(new Pe(j,
	                    new PeProvisionerSimple(Constants.HOST_MIPS[hostType])));
	            }
	...
	public static void printResults(
		...
		double perfSlaTimeHighUtil = getPerfSlaTime(hosts);

		double sla = slaTimePerActiveHost * slaDegradationDueToMigration ;
		double perfSlaPenalty = 0.5 * perfSlaTimeHighUtil;
		// NEW: Memory SLA time and half-penalty
		double memorySlaTime = getMemorySlaTime(vms);
		double memorySlaPenalty = memorySlaTime;

		// Final SLA (clamped)
		sla = Math.min(1.0, sla + perfSlaPenalty + memorySlaPenalty);

		// Final SLA = base + half-weighted performance penalty (clamped to [0,1])
		...
		if (outputInCsv) {
			...
			data.append(String.format("%.20f", memorySlaTime) + delimeter);
			data.append(String.format("%.20f", memorySlaPenalty) + delimeter);
			...
			if (datacenter.getVmAllocationPolicy() instanceof PowerVmAllocationPolicyMigrationAbstract) {
				...
				data.append(String.format("%.10f", perfSlaTimeHighUtil) + delimeter);
				data.append(String.format("%.10f", perfSlaPenalty) + delimeter);
			...
			Log.printLine(String.format("Memory SLA penalty: %.4f%%", memorySlaPenalty * 100));
			...
			Log.printLine(String.format("Perf SLA penalty: %.4f%%", perfSlaPenalty * 100));
			Log.printLine(String.format("TOTAL SLA violation: %.4f%%", sla * 100));
			if (datacenter.getVmAllocationPolicy() instanceof PowerVmAllocationPolicyMigrationAbstract) {
				PowerVmAllocationPolicyMigrationAbstract vmAllocationPolicy = (PowerVmAllocationPolicyMigrationAbstract) datacenter
						.getVmAllocationPolicy();
	...
	// Memory SLA violation Calculation - VM Type
	protected static int getVmType(Vm vm) {
	    for (int t = 0; t < Constants.VM_TYPES; t++) {
	        boolean mipsMatch = Math.abs(vm.getMips() - Constants.VM_MIPS[t]) < 1e-6;
	        boolean pesMatch  = vm.getNumberOfPes() == Constants.VM_PES[t];
	        if (mipsMatch && pesMatch) {
	            return t; // Found matching type (0,1,2,...)
	        }
	    }

	    // No match found — throw an error to prevent silent misclassification
	    throw new IllegalArgumentException(
	        String.format(
	            "Unable to determine VM type for VM ID %d: mips=%.2f, pes=%d",
	            vm.getId(),
	            vm.getMips(),
	            vm.getNumberOfPes()
	        )
	...
	//Memory SLA Time
	// Computes fraction of total VM-time where memory utilization exceeds 100%,
	// using per-type scaling: VM1=1.2x, VM2=1.0x, VM3=0.9x of CPU utilization fraction.
	private static final double[] MEM_ALPHA = {
		...
		private static final double DEFAULT_MEM_ALPHA = 1.0;
		private static final double EPS = 1e-9;

		protected static double getMemorySlaTime(final List<Vm> vms) {
		    return getMemorySlaTime(vms, MEM_ALPHA);
		}
	
		protected static double getMemorySlaTime(final List<Vm> vms, final double[] memAlphaByType) {
		    ...
		    for (Vm vm : vms) {
		        ...
		        // Resolve VM type and α safely
		        final int t = getVmType(vm); // must return 0..4 for your 5 types
		        final double alpha = alphaForType(t, memAlphaByType);

		        double prevTime = Double.NaN;
		        double prevRequested = 0.0;

		        for (VmStateHistoryEntry e : vm.getStateHistory()) {
		            ...
		            if (!Double.isNaN(prevTime)) {
		                ...
		                if (dt > 0) {
		                    ...
		                    // Requested CPU as fraction of VM capacity during [prevTime, time)
		                    double cpuFrac = (vmMaxMips > 0.0) ? (prevRequested / vmMaxMips) : 0.0;
		                    if (cpuFrac < 0) cpuFrac = 0.0; // clamp

		                    // Infer memory fraction using α for this VM type
		                    final double memFrac = alpha * cpuFrac;

		                    // Count interval as violating if inferred memory > 100%
		                    if (memFrac > 1.0 + EPS) {
		                        violationTime += dt;
		                    }
		                }
		            }
		            prevRequested = e.getRequestedMips();
		            prevTime = time;
		        }
		    }
		    return (totalTime == 0.0) ? 0.0 : (violationTime / totalTime);
		}

		private static double alphaForType(final int type, final double[] memAlphaByType) {
		    if (type >= 0 && type < memAlphaByType.length) {
		        final double a = memAlphaByType[type];
		        if (!Double.isNaN(a) && a > 0.0) return a;
		    }
		    return DEFAULT_MEM_ALPHA; // safe fallback
		}
	...
	// This part is calculatig the proposed Performance SKA Violation when latency go beyond accpetable limits due ti high context switching and other paramaters
	protected static double getPerfSlaTime(List<Host> hosts) {
	    final double LOWER = 0.85;
	    final double UPPER = 1.0 + 1e-9; // inclusive of 1.0 with tiny epsilon
	    double highUtilTime = 0;
	    double totalActiveTime = 0;

	    for (Host _host : hosts) {
	        HostDynamicWorkload host = (HostDynamicWorkload) _host;

	        // total host capacity in MIPS
	        double hostTotalMips = 0;
	        for (Pe pe : host.getPeList()) {
	            hostTotalMips += pe.getPeProvisioner().getMips();
	        }

	        double prevTime = -1;
	        double prevRequested = 0;
	        double prevAllocated = 0;
	        boolean prevActive = true;

	        for (HostStateHistoryEntry e : host.getStateHistory()) {
	            if (prevTime != -1) {
	                double dt = e.getTime() - prevTime;
	                if (prevActive) {
	                    totalActiveTime += dt;

	                    // utilization based on requested (what workloads want)
	                    double util = hostTotalMips > 0 ? prevRequested / hostTotalMips : 0;

	                    // count only when meeting demand AND in [85%, 100%]
	                    if (prevAllocated >= prevRequested && util >= LOWER && util <= UPPER) {
	                        highUtilTime += dt;
	                    }
	                }
	            }
	            prevTime = e.getTime();
	            prevRequested = e.getRequestedMips();
	            prevAllocated = e.getAllocatedMips();
	            prevActive = e.isActive();
	        }
	    }

	    if (totalActiveTime == 0) return 0;
	    return highUtilTime / totalActiveTime; // fraction of active time in the 85–100% band
	}
